...
**Deployment model.** Kanto is a hybrid system. Everything except Ditto runs on Oracle Kubernetes Engine (OKE) with Oracle Object Storage (OS) and Oracle Streaming as the data and event layers. Ditto runs on Modal because the GPU workload is bursty and Modal's per-second billing eliminates the cost of always-on GPU nodes.
...
- Persist per-protein embeddings to OS as Parquet for on-demand drilldown.
...
- Per-protein global similarity search index — keep per-protein on OS, build temporary FAISS indexes at investigation time.
...
| **Total per-protein Parquet on OS (backfill)** | **~14 TB** |
...
| Protein FASTA (Prodigal output) | OS (`os://kanto-proteins/{accession}/{version}.faa.gz`) | We computed it; needed for re-embedding when ESM C upgrades; also the handoff mechanism to Ditto. |
| Per-protein embeddings | OS as Parquet (`os://kanto-embeddings/{accession}/{version}.parquet`) | Too large to index globally; loaded on demand for drilldown. |
...
Mirroring your logging system, Kanto has four logical planes, with the addition of a hybrid compute split between OKE and Modal.

```
┌─────────────────────────────────────────────────────────────┐
│  CONTROL PLANE          (Growlithe, on OKE)                  │
│  Polls NCBI FTP, emits "new isolate" events                  │
└─────────────────────────────────────────────────────────────┘
                             │
                             ▼
┌─────────────────────────────────────────────────────────────┐
│  EVENT PLANE            (OCI Streaming + Modal queue)        │
│  OCI Streaming: small pointer messages between OKE services  │
...
┌─────────────────────────────────────────────────────────────┐
│  COMPUTE PLANE                                               │
│  OKE: Snorlax (CPU) → Alakazam → Chatot                      │
│  Modal: Ditto (GPU)                                          │
└─────────────────────────────────────────────────────────────┘
                             │
                             ▼
┌─────────────────────────────────────────────────────────────┐
│  STORAGE & QUERY PLANE  (OS + Mew + API, on OCI)             │
│  OS: protein FASTAs, per-protein Parquet                     │
│  Mew: metadata, per-genome embeddings, alerts                │
└─────────────────────────────────────────────────────────────┘
```

**Why no Kafka.** At Kanto's scale (thousands of messages per day, not per second), Kafka's strengths are not load-bearing. OCI Streaming gives us at-least-once delivery, consumer groups, replay, and partition-based fan-out — the Kafka-API-compatible properties we actually need — without the operational cost of running our own broker cluster on OKE. The team's Kafka knowledge transfers directly because the consumer protocol is the same.

**Why Modal for Ditto.** Embedding is bursty (daily NCBI publishes arrive in clumps) and the GPU is our most expensive resource by an order of magnitude. An always-on L40S on OKE would be ~14% utilized at steady state. Modal's per-second billing means we only pay for the ~5 GPU-hours/day we actually use, and elastic scaling for backfill bursts is a config change rather than a Karpenter dance.

---

## 6. Pipeline stages

Five services, three OCI Streaming streams, one Modal app. Streams carry only small pointer messages (under 4 KB each); all bulk data flows through Object Storage.

```
        ┌───────────────────────────┐
        │  Growlithe (Deployment)   │  Polls NCBI daily, on OKE
        └─────────────┬─────────────┘
                      │ writes IsolateDiscovered (~200 bytes)
                      ▼
            ┌────────────────────────┐
            │  kanto.discovered      │  OCI Streaming
            └────────────┬───────────┘
                         │
                         ▼
        ┌───────────────────────────┐
        │  Snorlax (StatefulSet)    │  CPU pod, on OKE
        │  ┌─────────────────────┐  │
        │  │ 1. Download FASTA   │  │  ← from NCBI FTP
        │  │ 2. Run Prodigal     │  │  ← in same pod
        │  │ 3. Write to OS      │  │  ← protein FASTA
        ...
        │  1. Read FASTA from OS    │
        │  2. ESM C 600M inference  │
        │  3. Write Parquet to OS   │  ← per-protein embeddings
        ...
        └─────────────┬─────────────┘
                      ...
                      ▼
            ┌────────────────────────┐
            │  kanto.embedded        │  OCI Streaming
            └────────────┬───────────┘
                         │
                         ▼
        ┌───────────────────────────┐
        │  Alakazam (Deployment)    │  on OKE
        ...
        └─────────────┬─────────────┘
                      ...
                      ▼
            ┌────────────────────────┐
            │  kanto.scored          │  OCI Streaming
            └────────────┬───────────┘
                         │
                         ▼
        ┌───────────────────────────┐
        │  Chatot (Deployment)      │  on OKE
        │  Filters above threshold, │
        │  dispatches notifications │
        └───────────────────────────┘
```

### 6.1 Growlithe (Discoverer) — on OKE

A single Deployment, not a StatefulSet — no per-partition state. Runs a CronJob every 6 hours.

**What it does.** Lists NCBI Pathogen Detection FTP, downloads the latest metadata TSV per organism, diffs against the previous run's TSV (cached in OS — metadata only, ~tens of MB), and writes one `IsolateDiscovered` event per new PDT accession to the `kanto.discovered` stream.
...
### 6.2 Snorlax (Ingester + Gene Predictor) — on OKE, colocated
...
5. Writes the compressed protein FASTA to OS at `os://kanto-proteins/{accession}/{version}.faa.gz`.
...
**Why directly invoke Modal instead of writing to a stream?** Modal's internal queue handles the OKE → Modal handoff cleanly. Adding an intermediate OCI Streaming topic would be double-queuing — Modal still queues the call internally even if we have a stream in front of it. The cost is one architectural inconsistency: streams are used between OKE services, Modal SDK is used to invoke Modal. The boundary is clear and well-documented; juniors won't get confused.
...
**Failure handling.** If the OS write succeeds but the Modal spawn fails, Snorlax retries the spawn (the OS object is the durable artifact; the Modal call is just an invocation). If both fail, the OCI Streaming consumer offset is not advanced and Snorlax will reprocess on retry. All operations are idempotent on `(accession, version)`.
...
**What it does:**
1. Fetches the protein FASTA from OS using the provided key. Uses an OCI SDK with credentials from a Modal secret.
...
7. **Writes per-protein embeddings to OS** as Parquet at `os://kanto-embeddings/{accession}/{version}.parquet`.
...
image = (
    ...
    .pip_install("esm", "torch", "oci", "psycopg[binary]", "pyarrow")
...
@app.function(
    ...
    gpu="L40S",
    secrets=[
        modal.Secret.from_name("oci-credentials"),
...
**Idempotency.** We pass `accession-version` as a Modal idempotency key. Modal deduplicates concurrent calls with the same key. Inside Ditto, all writes are idempotent (OS overwrites are no-ops, Mew uses `INSERT ... ON CONFLICT`).
...
**Network path.** Modal can run in regions co-located with OCI; check current Modal region availability. If there's no Modal region in the same OCI region, egress from OCI Object Storage to Modal applies. OCI's outbound egress is generally cheaper than AWS, but verify current rates — at 14 TB of total backfill data, even small per-GB charges add up.

**Mew connectivity.** Mew exposes a TLS-enabled Postgres endpoint with cert-based auth. Modal connects directly. We do not put a proxy service in front — that'd be over-engineering for v1. Network ACLs on the Mew endpoint restrict access to Modal's egress IP ranges plus the OKE cluster's NAT IPs.

### 6.4 Alakazam (Scorer) — on OKE
...
**Tiered scoring.** ~95% of isolates get only the cheap genome-level check; ~5% trigger the full coverage check. Keeps Alakazam efficient and OS reads minimal.

### 6.5 Chatot (Alerter) — on OKE
...
Mew is a single PostgreSQL 16 instance with the pgvector extension, hosted on OCI Database for PostgreSQL. It holds:
...
**Connectivity.**
- OKE services connect via the OCI VCN's private endpoint.
...
| Snorlax → OS | ~500 KB–2 MB compressed protein FASTA | OS PUT | Yes (`kanto-proteins` bucket) |
| Snorlax → Modal | ~200 bytes (function args with OS key) | Modal SDK | Modal queue (transient) |
| OS → Ditto (on Modal) | ~500 KB–2 MB | OS GET (cross-cloud or same-region) | N/A |
| Ditto → OS | ~7 MB Parquet (per-protein embeddings) | OS PUT | Yes (`kanto-embeddings` bucket) |
| Ditto → Mew | 2.3 KB (per-genome aggregate) | Postgres INSERT over TLS | Yes (pgvector index) |
| Ditto → OCI Streaming | ~200 bytes | stream produce | retained 7 days |
| Streaming → Alakazam | ~200 bytes | stream consume | N/A |
| Alakazam → Mew | k-NN query + UPDATE | SQL | Yes |
| Alakazam → OS (only ~5%) | Read ~7 MB Parquet | OS GET, gated | N/A |
| Alakazam → OCI Streaming | ~300 bytes | stream produce | retained 7 days |
| Streaming → Chatot | ~300 bytes | stream consume | N/A |
| Chatot → outbound | tens of bytes per channel | webhook/email | external |

Total bytes leaving Snorlax per isolate: **roughly 1–2 MB** (one OS write, one tiny Modal call).
Total OS writes per isolate: **2** (Snorlax writes protein FASTA, Ditto writes embedding Parquet).
Total OS reads per isolate on hot path: **1** (Ditto reads protein FASTA on Modal). Plus the optional Alakazam coverage check on ~5% of isolates.
...
1. **Bulk download** via NCBI Datasets CLI in batches of ~10,000, parallelized across 16 worker pods on OKE, rate-limited to ~10 concurrent FTP connections (be a polite citizen).
...
- OCI Streaming consumer offsets commit only after OS write + Mew update + downstream stream produce all succeed.
- OS writes are idempotent: same accession+version overwrites are no-ops semantically.
...
- OS write succeeds, Modal spawn fails: Snorlax retries the spawn. The OS object is the durable artifact. Eventually succeeds or goes to DLQ.
- OS write fails: Snorlax does not spawn Modal call. Stream offset not advanced; consumer retries the whole download+Prodigal. This is the safest failure mode because Prodigal is fast.
- Both succeed but the Mew status update fails: the isolate is in OS and in Modal's queue but Mew doesn't know. On Snorlax retry, the dedup check on `(accession, version)` in Mew shows no row, so it reprocesses — but Modal dedups the call, so we don't waste GPU. End state is correct.
...
**Model upgrades.** When ESM C v2 ships, we kick off a backfill job that reads protein FASTAs from OS (the durability copy) and re-embeds via the same Modal function but with a new model version. New embeddings populate a new pgvector table; reads cut over atomically.
...
| **Producer-Consumer** | OCI Streaming between OKE stages | Standard staged pipeline; mirrors logging system patterns. |
...
| **Repository** | Data access layer over Mew + OS | One abstraction for "fetch isolate X with all its data"; trivially testable. |

Patterns explicitly *not* used:
- **Mediator** — OCI Streaming already mediates.
...
- **Ditto isolated on Modal** because its resource profile (bursty GPU) is fundamentally different from everything else. Hybrid OKE + Modal is two deployment systems but one logical service per component.
...
Running Ditto on Modal while everything else runs on OKE introduces real complexity:

- **Two CI pipelines.** OKE manifests deploy via your existing pipeline; Modal apps deploy via `modal deploy`.
- **Split observability.** Modal logs live in Modal's UI; we ship them to OCI Logging via Modal's log forwarding to keep one place to look.
- **Cross-cloud secrets.** Modal needs OCI credentials (for OS access) and Mew credentials. Stored as Modal Secrets.
- **Cross-cloud network.** OS reads and Mew writes from Modal go over the public internet (TLS-secured). Egress from OCI to Modal is a recurring cost we monitor.

We accept all of this because GPU economics are favorable enough to make it worthwhile. A v2 review will revisit whether to bring Ditto back onto OKE GPU nodes if scale changes the calculus.
...
- **Bring Ditto back onto OKE GPU nodes** if scale changes the GPU economics or cross-cloud egress becomes painful.
- **Move Mew to private endpoint only** by introducing an OKE-side proxy service that Modal calls over a more locked-down protocol. Defer until security review demands it.
