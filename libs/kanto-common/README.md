...
| Concern         | Module                                              |
| --------------- | --------------------------------------------------- |
| Event schemas   | `kanto_common.schemas`                              |
| Object Storage  | `kanto_common.storage`                              |
| Mew (Postgres)  | `kanto_common.mew`                                  |
| OCI Streaming   | `kanto_common.streaming`                            |
| Logging         | `kanto_common.logging`                              |
| Tracing         | `kanto_common.tracing`                              |
| Configuration   | `kanto_common.config`                               |
| Test helpers    | `kanto_common.testing`                              |

The most-used types are re-exported from the top level; service code
should usually `from kanto_common import IsolateDiscovered, ...`
...
```python
from kanto_common.streaming import StreamingProducer, StreamingConsumer, lifecycle
...
```python
from kanto_common.storage import ObjectStorageClient, KeyBuilder
...
in `kanto_common.mew.repositories`; service code never writes SQL.
...
`kanto_common.testing.bootstrap_schema` — see "Testing helpers" below.
...
```python
from kanto_common.config import KantoBaseSettings
...
```python
from kanto_common.logging import setup_logging, get_logger, bind_accession
...
```python
from kanto_common.tracing import setup_tracing, span, use_extracted_context
...
`from kanto_common.testing import ...` gives services:
...
uv run pytest libs/kanto-common/tests/        # unit tests
uv run pytest libs/kanto-common/tests/ -m integration   # needs Docker
uv run mypy libs/kanto-common/src/kanto_common
```
