...
3. **DLQ routing** — when a callback raises after the configured
   ...
   :class:`~kanto_common.schemas.DLQEnvelope` to ``<topic>.dlq`` and
...
from kanto_common.config import StreamingSettings
...
from kanto_common.tracing import (
...
class StreamingConsumer:
    ...
    async def messages(self) -> AsyncIterator[ConsumedMessage[KantoEvent]]:
        ...
        :func:`kanto_common.tracing.use_extracted_context` and the
